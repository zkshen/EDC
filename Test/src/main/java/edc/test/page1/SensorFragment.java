...
public class SensorFragment extends Fragment {
        ...
        public void handleMessage(Message msg) {
            ...
            switch (msg.what) {
                ...
                case Constants.MESSAGE_WRITE:
                    ...
                    State.setText( writeMessage);
                    break;
                case Constants.MESSAGE_READ:
                    byte[] readBuf = (byte[]) msg.obj;
                    // construct a string from the valid bytes in the buffer
                    final String strHumidity = new String(readBuf, 10, 6);  //截取不为0
                    final String strTemperature = new String(readBuf, 31, 5);
                    final String strPressure= new String(readBuf, 49, 7);
    ...
    public void onActivityResult(int requestCode, int resultCode, Intent data) {
        switch (requestCode) {
            case REQUEST_CONNECT_DEVICE_SECURE:
                // When SensorListActivity returns with a device to connect
     ...
     *
     * @param data   An {@link Intent} with {@link SensorListActivity#EXTRA_DEVICE_ADDRESS} extra.
     * @param secure Socket Security type - Secure (true) , Insecure (false)
     */
    private void connectDevice(Intent data, boolean secure) {
        // Get the device MAC address
        String address = data.getExtras()
                .getString(SensorListActivity.EXTRA_DEVICE_ADDRESS);
